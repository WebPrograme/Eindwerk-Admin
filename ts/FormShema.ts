import type { Config, Input, Requirements, Type, Receipt } from './types';

function CheckSubmit(config: Config) {
	CheckInputs({
		...config,
		showErrors: false,
		update: false,
...
function CheckInputs(config: Config) {
	let submit = null;
	let unValidInputs: Input[] = [];
	let abortController = new AbortController();
	let allListenersAndObservers: Receipt = { inputs: [], abortController: abortController };
	...
	config.inputs.forEach((input) => {
		...
		if (config.update && hasHidden) {
			...
			const inputElement: HTMLElement = document.querySelector(hiddenElement);
	...
	if (config.update) {
		config.additionalTriggers?.forEach((trigger) => {
			if (document.querySelector(trigger.selector).nodeName == 'TEXTAREA') {
				document.querySelector(trigger.selector).addEventListener(
					'keypress',
					() => {
						CheckInputs({
							...config,
		...
		config.inputs.forEach((input: Input<Type>) => {
			if (document.querySelector(input.selector).nodeName == 'TEXTAREA') {
				document.querySelector(input.selector).addEventListener(
					'keypress',
					() => {
						CheckInputs({
							...config,
			...
			if (input.requirements && input.requirements.type == 'select') {
				const requirements: Requirements<'select'> = input.requirements as Requirements<'select'>;
				const options = requirements.options as { [key: string]: Requirements<Type> };

				Object.keys(options).forEach((select) => {
					const selectRequirement = options[select] as Requirements<'inputs'>;
...
function TriggerUpdateEvents(config: Config, update: boolean = false) {
	CheckInputs({
		...config,
		update: update,
	});
...
function getValue(selector: string) {
	...
	if (element.nodeName == 'INPUT' && (element as HTMLInputElement).type == 'file') {
		return (element as HTMLInputElement).files[0] ? (element as HTMLInputElement).files[0].name : element.dataset.src;
...
function validateInput(input: Input<Type>, showErrors: boolean) {
	...
	if (hasRequirements) {
		...
		valid = checkRequirments(requirement, value, inputElement as HTMLElement);
	} else {
		if (value == '' || value == null) {
			valid = false;
		}
		...
		if (!valid) {
			...
			if (input.requirements && input.requirements.type == 'select') {
				...
				Object.keys(options).forEach((select) => {
					...
					inputs.forEach((input) => {
						const element: HTMLElement = document.querySelector(input.selector);
...
function checkRequirments(requirements: Requirements<Type>, value: any, inputElement: HTMLElement) {
	let valid = true;
...
function GetInputValues(config: Config) {
	let values: { [key: string]: any } = {};

	config.inputs.forEach((input) => {
		const inputElement = document.querySelector(input.selector) as HTMLInputElement;
...
function StopUpdate(allListenersAndObservers: Receipt) {
